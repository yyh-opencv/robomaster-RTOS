...
int16_t PID_arm(Error *arm_error,int16_t speed_now,int16_t speed_exp,float kp,float ki) 
{
	arm_error->Now =speed_exp - speed_now;
	
	arm_error->Last_out = kp * (arm_error->Now - arm_error->Last ) //kp乘(本次误差 - 上次误差）
	                      + ki * arm_error->Now;  //ki乘本次误差
	                   
	
	
	arm_error->Last = arm_error->Now;//保存上一次偏差
	arm_error->Now_out += arm_error->Last_out;//增量输出
	
	
	
	if(arm_error->Now_out > 8000) arm_error->Now_out = 8000;//限制上限，防止超出PWM量程
	if(arm_error->Now_out <-8000) arm_error->Now_out =-8000;

	return arm_error->Now_out;//返回输出值
}
